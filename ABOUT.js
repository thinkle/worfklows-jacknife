...
The basic idea here is that we create a master configuration sheet
that handles a number of kinds of actions. These actions can be strung
together into a workflow. The spreadsheet manages everything and can
be edited directly as needed or via a web interface.

The sidebar can be used to create the configuration sheets as needed
and associate them with forms.

All of the configuration data for the program lives in that
spreadsheet.
...
Approval - this sets up an approval form workflow. You start with a
form and then you create a near duplicate where someone else
"approves" the form. When the first form is completed, it triggers an
approval email going to the second email.

- This will create tabs based on the name of the initial form
- NAME Form Approval Form -> this will have the new forms edit URL and
  it also has a list of fields that get changed between one form and
  another (as opposed to fields that get copied directly)
...
Email by Field - this is a simple trigger to generate an email based
on form results when a form is submitted.
